...
export default function UserPanel() {
  ...
  const [misSolicitudes, setMisSolicitudes] = useState({}) // Mantiene tu lógica indexada por clubId
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // CARGAR ESTADO REAL DE CADA CLUB (APROBADOS)
        const estadoMiembro = {}
  ...
  return (
    <section className="min-h-screen bg-[#f8f9fa] py-8 px-4 font-sans sm:px-6">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {/* --- HEADER CORREGIDO CON SENTIDO NATURAL --- */}
        ...
        {/* --- ALERTAS DE FEEDBACK ESTILIZADAS --- */}
        {mensajeFeedback && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {clubes.map(club => {
            ...
            return (
              ...
              >
                <div>
                  {/* CONTENEDOR PORTADA (CON BLUR EFECTO ESPEJO) */}
                ...
                <div className="px-5 pb-5 pt-2 flex gap-2">
                  ...
                  <button
                    ...
                    disabled={isLoading || estado === "Pendiente" || estado === "Aprobado"}
                    className={`flex-1 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider flex justify-center items-center gap-2 border transition duration-200 active:scale-98 ${
                      estado === "Aprobado"
                        ...
                        : "bg-[#2c3e50] text-white border-transparent hover:bg-[#e67e22] shadow-3xs cursor-pointer"
                    }`}
                  >
                    {isLoading ? (
                      ...
                      <>
                        Eres miembro <MdCheckCircle size={14} />
                      </>
                    ) : estado === "Pendiente" ? (
                      <>
                        Pendiente <MdAccessTime size={14} />
                      </>
                    ) : (
                      <>
                        Unirse <MdChevronRight size={16} />
                      </>
                    )}
                  </button>
                </div>
